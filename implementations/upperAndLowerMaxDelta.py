...
# This works by looking forward into the next sevendays and picking the largest delta.
# This ignores days in which the delta is greater than 'highestDelta'. This is to ensure we don't try to train on unusual day swings
# Any day with delta higher than 'deltaLimit' will be set to some max value



tickerData = load_time_series_daily("MMM")




def getDayOutlook(tickerData, earliestDay, dayCount = 14):
    '''
    ...
    for day in tickerData:
        ...
        highestDelta = 0
        ignoreDay = False
        while counter < dayCount and len(nextDaySeriesIndexes)<10:
            ...
            if dayIndex in tickerData:
                ...
                if highestDelta > 4:#ignore days with higher than 4% swings
                    ignoreDay = True
                    break
        ...
        if not ignoreDay:
            ...
            if len(precedingNintyDays) > 30:
                ...
                peakDelta = deltaLimit if  highestDelta >= deltaLimit else highestDelta
                retValue[day] = {
